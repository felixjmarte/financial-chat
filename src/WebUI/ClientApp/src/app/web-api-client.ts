...
export interface IChatMessagesClient {
    send(command: SendChatMessageCommand): Observable<number>;
}

@Injectable({
    providedIn: 'root'
})
export class ChatMessagesClient implements IChatMessagesClient {
    ...
    send(command: SendChatMessageCommand): Observable<number> {
        let url_ = this.baseUrl + "/api/ChatMessages/Send";
        url_ = url_.replace(/[?&]$/, "");
...
export class ChatRoomsClient implements IChatRoomsClient {
    ...
    get2(chatRoomCode: string | null): Observable<ChatRoomsVm[]> {
        ...
        url_ = url_.replace("{chatRoomCode}", encodeURIComponent("" + chatRoomCode));
        url_ = url_.replace(/[?&]$/, "");
